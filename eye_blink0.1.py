...
def draw_eye(surface, eye, dt):
    ...
    # --- Eyelashes (top) — hidden during blink ---
    if not eye["blinking"]:
        num_lashes = 9
        for i in range(num_lashes):
            ...
            # Outward normal: atan2 from centre to base point gives the exact
            # radial direction, perpendicular to the ellipse surface.
            ...
            pygame.draw.line(surface, LASH_COLOR, (int(bx), int(by)),
                             (int(ex), int(ey)), 2)
